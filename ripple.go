...
// Build a new application object.
func NewApplicationByStreamContentType() *Application {
	output := new(Application)
	output.controllers = make(map[string]interface{})
	...
	for routeIndex := 0; routeIndex < len(this.routes); routeIndex++ {
		...
		for i := 0; i < len(patternTokens); i++ {
			...
			if patternToken == ":_controller" {
				...
				parametersTokens := strings.Split(pathToken, "&")
				for _, value := range parametersTokens {
					paramPair := strings.Replace(value, "=", "&", 1)
					parameterField := strings.Split(paramPair, "&")
					if 0 == len(parameterField) { //判断URL链接是否存在格式错误
						continue
					}
					params[parameterField[0]] = parameterField[1]
				}
			} else {
				notMatched = true
				break
